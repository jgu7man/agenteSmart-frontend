...
export class RespuestasService {
  ...
  async responsesRef() {
    ...
    let mensajeName = this._mensaje.current$.getValue().name;
  ...
  async setRespuesta(respuesta: RespuestaModel) {
    if (respuesta.id) {
      ...
      if (
        ...
        let res = await (await this.responsesRef()).add(respuesta);
        await (await this.responsesRef()).doc(res.id).update({ id: res.id });
      }
    }
  ...
  async delRespuesta(respuestaId: string) {
    try {
      await (await this.responsesRef()).doc(respuestaId).delete();
