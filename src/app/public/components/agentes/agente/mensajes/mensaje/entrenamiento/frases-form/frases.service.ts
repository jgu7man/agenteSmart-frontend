...
export class FrasesService {
  mensajesPath: string;
  list$: BehaviorSubject<FraseEntrenamiento[]> = new BehaviorSubject([]);
  ...
  constructor(
    ...
    this._mensaje.current$.pipe(
      map((mensaje) => mensaje.trainingPhrases)
    ).subscribe(this.list$)
  }
  ...
  async addTraningPhrase(frase: FraseEntrenamiento, index?: number) {
    try {
      ...
      if (trainingPhrases.length > 0) {
        index
          ? trainingPhrases.splice(index, 0, frase)
          : trainingPhrases.push(frase);
      } else {
        trainingPhrases.push(frase)
      }
  ...
  async stractSelectedPart(
    ...
    try {
      ...
      initialParts.forEach((p, i) => {
        if (p.text.includes(textSelected)) {
          partSelected = [i, p];
        }
      });
      ...
      if (partSelected) {
        ...
        let newParts: Map<number, FraseParte> = await this.getTextSelectInPart(
          partSelected[1].text,
          textSelected
        );
  ...
  findFraseIndex(frase: FraseEntrenamiento) {
  ...
  async deletePhrase(frase: FraseEntrenamiento) {
    try {
      const mensaje = this._mensaje.current$.getValue();
      const trainingPhrases = mensaje.trainingPhrases;
      const phraseToDel = trainingPhrases.findIndex(
        (phrase) => phrase.name === frase.name
      );
      trainingPhrases.splice(phraseToDel, 1);
